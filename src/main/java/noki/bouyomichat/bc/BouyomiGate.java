...
public class BouyomiGate {
	...
	private class BouyomiData {
		...
		private void postReal(EBouyomiPostType type) throws IOException {
			...
			try {
				socket = new Socket();
				socket.connect(new InetSocketAddress(BouyomiChatConf.host.get(), BouyomiChatConf.port.get()));
		...
		private void postReal(EBouyomiPostType type, String message) throws IOException {
			...
			try {
				...
				DataOutputStream output = new DataOutputStream(socket.getOutputStream());
				output.writeShort(Short.reverseBytes(type.getCommand()));
				output.writeShort(Short.reverseBytes(BouyomiChatConf.speed.get().shortValue()));
				output.writeShort(Short.reverseBytes(BouyomiChatConf.tone.get().shortValue()));
				output.writeShort(Short.reverseBytes(BouyomiChatConf.volume.get().shortValue()));
				output.writeShort(Short.reverseBytes(BouyomiChatConf.voice.get().shortValue()));
				output.write(0);
